...
const SkillBadge: React.FC<SkillBadgeProps> = ({ skill, icon, delay }) => {
  return (
    ...
    >
      <Badge
        variant="secondary"
        className="flex items-center gap-2 px-3 py-2 text-sm bg-background/50 backdrop-blur-sm border-border/50 hover:bg-background/80 transition-colors"
      >
        {icon}
        {skill}
      </Badge>
...
const DeveloperHero: React.FC = () => {
  ...
  const skills = [
    ...
    { name: "Generative AI", icon: <Terminal className="w-4 h-4" /> },
    { name: "Langgraph and LangChain", icon: <Terminal className="w-4 h-4" /> },
  ...
  return (
    ...
    >
      ...
      <FloatingElement
        delay={1}
        className="absolute top-40 right-20 opacity-20 pointer-events-none"
      >
      ...
      <FloatingElement
        delay={2}
        className="absolute bottom-32 left-20 opacity-25 pointer-events-none"
      >
      ...
      >
        <div className="max-w-4xl mx-auto text-center space-y-8">
          ...
          >
            Experienced in building scalable AI Products and real-time
            applications. Passionate about creating efficient, Robust and Research based AI Products.
          </motion.p>
